...
import StylableLabel, { TmpOverflow } from "../StylableLabel";
...
export default class TmpAssembler {
    ...
    static createData(comp: StylableLabel) {
        return comp.requestRenderData();
    }

    static fillBuffers(comp: StylableLabel, renderer: any) {
     ...
     */
    public static updateRenderData(comp: StylableLabel): void {
        if (!comp.renderData) {
            return;
        }
        if (_comp === comp) {
    ...
    static updateColor(comp: StylableLabel) {
        if (JSB) {
            const renderData = comp.renderData!;
    ...
    static updateUVs(comp: StylableLabel) {
        const renderData = comp.renderData!;
        const vData = renderData.chunk.vb;
        const vertexCount = renderData.vertexCount;
    ...
    static updateTextureIdx(comp: StylableLabel) {
        const renderData = comp.renderData!;
        const vData = renderData.chunk.vb;
        const vertexCount = renderData.vertexCount;
        const dataList = renderData.data;
     ...
     */
    private static _updateProperties(comp: StylableLabel): void {
    ...
    private static _multilineTextWrap(nextTokenFunc: Function): boolean {
        ...
        for (let index = 0; index < textLen; ) {
            ...
            for (let tmp = 0; tmp < tokenLen; ++tmp) {
                ...
                if ((_comp as StylableLabel).enableItalic) {
                    _italicVec.x = 0;
    ...
    private static _updateQuads(): void {
        ...
        for (let i = 0, l = _comp.lettersInfo.length; i < l; ++i) {
            ...
            if (_tmpUvRect.height > 0 && _tmpUvRect.width > 0) {
                ...
                if (
                    _extraLineDef &&
                    ((_comp as StylableLabel).enableUnderline ||
                        (_comp as StylableLabel).enableStrikethrough)
                ) {
        ...
        if (_extraLineDef) {
            ...
            if ((_comp as StylableLabel).enableUnderline) {
                this._updateLineQuads(
                    appX,
                    appY,
                    -_fontSize +
                        (_comp as StylableLabel).underlineOffset * _bmfontScale
                );
            }
            // Delete line

            if ((_comp as StylableLabel).enableStrikethrough) {
                this._updateLineQuads(
                    appX,
                    appY,
                    -_fontSize / 2 +
                        (_comp as StylableLabel).strikethroughOffset *
                            _bmfontScale
                );
            }
        }
     ...
     */
    public static updateColorExtra(comp: StylableLabel): void {
        const dataList = comp.renderData.data;
     ...
     */
    public static isVisble(comp: StylableLabel, index: number): boolean {
